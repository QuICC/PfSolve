...
VkResult createInstance(VkGPU* vkGPU, uint64_t sample_id) {
	...
	applicationInfo.pApplicationName = "PfSolve";
	applicationInfo.applicationVersion = (uint32_t)PfSolveGetVersion();
	applicationInfo.pEngineName = "PfSolve";
...
PfSolveResult findMemoryType(VkGPU* vkGPU, uint64_t memoryTypeBits, uint64_t memorySize, VkMemoryPropertyFlags properties, uint32_t* memoryTypeIndex) {
...
PfSolveResult allocateBuffer(VkGPU* vkGPU, VkBuffer* buffer, VkDeviceMemory* deviceMemory, VkBufferUsageFlags usageFlags, VkMemoryPropertyFlags propertyFlags, uint64_t size) {
	...
	if (res != VK_SUCCESS) return PFSOLVE_ERROR_FAILED_TO_CREATE_BUFFER;
	...
	if (res != VK_SUCCESS) return PFSOLVE_ERROR_FAILED_TO_ALLOCATE_MEMORY;
	res = vkBindBufferMemory(vkGPU->device, buffer[0], deviceMemory[0], 0);
	if (res != VK_SUCCESS) return PFSOLVE_ERROR_FAILED_TO_BIND_BUFFER_MEMORY;
	return resFFT;
}
#endif
PfSolveResult transferDataToCPU(VkGPU* vkGPU, void* cpu_arr, void* output_buffer, uint64_t transferSize) {
	...
	if (res != VK_SUCCESS) return PFSOLVE_ERROR_FAILED_TO_ALLOCATE_COMMAND_BUFFERS;
	VkCommandBufferBeginInfo commandBufferBeginInfo = { VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO };
	commandBufferBeginInfo.flags = VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT;
	res = vkBeginCommandBuffer(commandBuffer, &commandBufferBeginInfo);
	if (res != VK_SUCCESS) return PFSOLVE_ERROR_FAILED_TO_BEGIN_COMMAND_BUFFER;
	...
	if (res != VK_SUCCESS) return PFSOLVE_ERROR_FAILED_TO_END_COMMAND_BUFFER;
	VkSubmitInfo submitInfo = { VK_STRUCTURE_TYPE_SUBMIT_INFO };
	submitInfo.commandBufferCount = 1;
	submitInfo.pCommandBuffers = &commandBuffer;
	res = vkQueueSubmit(vkGPU->queue, 1, &submitInfo, vkGPU->fence);
	if (res != VK_SUCCESS) return PFSOLVE_ERROR_FAILED_TO_SUBMIT_QUEUE;
	res = vkWaitForFences(vkGPU->device, 1, &vkGPU->fence, VK_TRUE, 100000000000);
	if (res != VK_SUCCESS) return PFSOLVE_ERROR_FAILED_TO_WAIT_FOR_FENCES;
	res = vkResetFences(vkGPU->device, 1, &vkGPU->fence);
	if (res != VK_SUCCESS) return PFSOLVE_ERROR_FAILED_TO_RESET_FENCES;
	...
	if (copyCommandBuffer == 0) return PFSOLVE_ERROR_FAILED_TO_CREATE_COMMAND_LIST;
	MTL::BlitCommandEncoder* blitCommandEncoder = copyCommandBuffer->blitCommandEncoder();
	if (blitCommandEncoder == 0) return PFSOLVE_ERROR_FAILED_TO_CREATE_COMMAND_LIST;
...
PfSolveResult transferDataFromCPU(VkGPU* vkGPU, void* cpu_arr, void* input_buffer, uint64_t transferSize) {
	PfSolveResult resFFT = PFSOLVE_SUCCESS;
#if(VKFFT_BACKEND==0)
	VkResult res = VK_SUCCESS;
...
PfSolveResult devices_list() {
	...
	if (res != VK_SUCCESS) return PFSOLVE_ERROR_FAILED_TO_CREATE_INSTANCE;

	uint32_t deviceCount;
	res = vkEnumeratePhysicalDevices(local_instance, &deviceCount, NULL);
	if (res != VK_SUCCESS) return PFSOLVE_ERROR_FAILED_TO_ENUMERATE_DEVICES;

	VkPhysicalDevice* devices = (VkPhysicalDevice*)malloc(sizeof(VkPhysicalDevice) * deviceCount);
	if (!devices) return PFSOLVE_ERROR_MALLOC_FAILED;
	if (devices) {
	...
	if (res != CUDA_SUCCESS) return PFSOLVE_ERROR_FAILED_TO_SET_DEVICE_ID;
	...
	for (uint64_t j = 0; j < numPlatforms; j++) {
		...
		for (uint64_t i = 0; i < numDevices; i++) {
			...
			if (res != CL_SUCCESS) return PFSOLVE_ERROR_FAILED_TO_ENUMERATE_DEVICES;
	...
	ze_result_t res = ZE_RESULT_SUCCESS;
	res = zeInit(0);
	if (res != ZE_RESULT_SUCCESS) return PFSOLVE_ERROR_FAILED_TO_INITIALIZE;
	...
	if (!drivers) return PFSOLVE_ERROR_MALLOC_FAILED;
	...
	for (uint64_t j = 0; j < numDrivers; j++) {
		...
		if (res != ZE_RESULT_SUCCESS) return PFSOLVE_ERROR_FAILED_TO_GET_DEVICE;
		...
		for (uint64_t i = 0; i < numDevices; i++) {
			...
			if (res != ZE_RESULT_SUCCESS) return PFSOLVE_ERROR_FAILED_TO_ENUMERATE_DEVICES;
...
PfSolveResult performVulkanFFT(VkGPU* vkGPU, PfSolveApplication* app, PfSolveLaunchParams* launchParams, int inverse, uint64_t num_iter, double* time_result) {
	PfSolveResult resFFT = PFSOLVE_SUCCESS;
#if(VKFFT_BACKEND==0)
	VkResult res = VK_SUCCESS;
	VkCommandBufferAllocateInfo commandBufferAllocateInfo = { VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO };
	...
	res = vkAllocateCommandBuffers(vkGPU->device, &commandBufferAllocateInfo, &commandBuffer);
	if (res != 0) return PFSOLVE_ERROR_FAILED_TO_ALLOCATE_COMMAND_BUFFERS;
	VkCommandBufferBeginInfo commandBufferBeginInfo = { VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO };
	commandBufferBeginInfo.flags = VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT;
	res = vkBeginCommandBuffer(commandBuffer, &commandBufferBeginInfo);
	if (res != 0) return PFSOLVE_ERROR_FAILED_TO_BEGIN_COMMAND_BUFFER;
	launchParams->commandBuffer = &commandBuffer;
	//Record commands num_iter times. Allows to perform multiple convolutions/transforms in one submit.
	for (uint64_t i = 0; i < num_iter; i++) {
		resFFT = PfSolveAppend(app, inverse, launchParams);
		if (resFFT != PFSOLVE_SUCCESS) return resFFT;
	}
	res = vkEndCommandBuffer(commandBuffer);
	if (res != 0) return PFSOLVE_ERROR_FAILED_TO_END_COMMAND_BUFFER;
	VkSubmitInfo submitInfo = { VK_STRUCTURE_TYPE_SUBMIT_INFO };
	submitInfo.commandBufferCount = 1;
	submitInfo.pCommandBuffers = &commandBuffer;
	std::chrono::steady_clock::time_point timeSubmit = std::chrono::steady_clock::now();
	res = vkQueueSubmit(vkGPU->queue, 1, &submitInfo, vkGPU->fence);
	if (res != 0) return PFSOLVE_ERROR_FAILED_TO_SUBMIT_QUEUE;
	res = vkWaitForFences(vkGPU->device, 1, &vkGPU->fence, VK_TRUE, 100000000000);
	if (res != 0) return PFSOLVE_ERROR_FAILED_TO_WAIT_FOR_FENCES;
	...
	res = vkResetFences(vkGPU->device, 1, &vkGPU->fence);
	if (res != 0) return PFSOLVE_ERROR_FAILED_TO_RESET_FENCES;
	vkFreeCommandBuffers(vkGPU->device, vkGPU->commandPool, 1, &commandBuffer);
#elif(VKFFT_BACKEND==1)
	cudaError_t res = cudaSuccess;
	std::chrono::steady_clock::time_point timeSubmit = std::chrono::steady_clock::now();
	...
	if (res != CL_SUCCESS) return PFSOLVE_ERROR_FAILED_TO_SYNCHRONIZE;
	std::chrono::steady_clock::time_point timeEnd = std::chrono::steady_clock::now();
	double totTime = std::chrono::duration_cast<std::chrono::microseconds>(timeEnd - timeSubmit).count() * 0.001;
#elif(VKFFT_BACKEND==4)
	ze_result_t res = ZE_RESULT_SUCCESS;
	...
	if (res != 0) return PFSOLVE_ERROR_FAILED_TO_DESTROY_COMMAND_LIST;
#elif(VKFFT_BACKEND==5)
...
PfSolveResult performVulkanFFTiFFT(VkGPU* vkGPU, PfSolveApplication* app, PfSolveLaunchParams* launchParams, uint64_t num_iter, double* time_result) {
	PfSolveResult resFFT = PFSOLVE_SUCCESS;
#if(VKFFT_BACKEND==0)
	VkResult res = VK_SUCCESS;
	VkCommandBufferAllocateInfo commandBufferAllocateInfo = { VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO };
	...
	launchParams->commandBuffer = &commandBuffer;
	for (uint64_t i = 0; i < num_iter; i++) {
		resFFT = PfSolveAppend(app, -1, launchParams);
		if (resFFT != PFSOLVE_SUCCESS) return resFFT;
		resFFT = PfSolveAppend(app, 1, launchParams);
		if (resFFT != PFSOLVE_SUCCESS) return resFFT;
	}
